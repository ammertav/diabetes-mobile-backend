<div class="col-span-12 lg:col-span-4" x-data="{
    ...
    <div id="preview-panel" x-show="selectedProtocol"
        class="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm sticky top-20">
        <!-- Header -->
        <div
            class="flex items-center justify-between mb-6 pb-3 border-b border-slate-100">
            <h5 class="text-base font-bold font-headline text-slate-900">Protocol
                Preview</h5>
            <span
        ...
        <form x-show="selectedProtocol"
            :action="'/fasting-protocols/' + selectedProtocol?.id" method="POST"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div x-show="activeEditField !== 'title'"
                    ...
                    <div class="flex items-center justify-between mb-2">
                        <span
                            :class="selectedProtocol?.type === 'sunnah' ?
                                'bg-amber-100 text-amber-900' : (
                                    selectedProtocol?.type === 'intermittent' ?
                    ...
                    <h4 class="text-2xl font-extrabold text-blue-600 font-headline tracking-tight"
                        x-text="selectedProtocol?.name"></h4>
                    <div class="flex items-center gap-1.5 mt-1">
                        <span
                            class="material-symbols-outlined text-sm text-emerald-600">verified</span>
                        <span
                            class="text-xs font-semibold text-emerald-700 font-body">Clinically
                            Validated</span>
                    </div>
                </div>
                ...
                <div x-show="activeEditField === 'title'"
                    ...
                    <div>
                        <label
                            class="text-[10px] font-bold text-slate-500 block mb-1">Tipe</label>
            ...
            <section>
                <div class="flex justify-between items-center mb-2">
                    <label
                        class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Fasting
                        Window</label>
                </div>

                <!-- Display Mode -->
                <div x-show="activeEditField !== 'window'"
                    @click="openEdit('window')"
                    ...
                    <div class="text-center">
                        <p
                            class="text-[11px] text-slate-400 font-semibold mb-0.5">
                            Start</p>
                        <p class="font-bold text-slate-900 text-sm font-headline"
                            x-text="selectedProtocol?.start_time || '18:00'">
                        </p>
                    </div>
                    ...
                    <div class="text-center">
                        <p
                            ...
                            End</p>
                        <p class="font-bold text-slate-900 text-sm font-headline"
                            x-text="selectedProtocol?.end_time || '10:00'"></p>
                    </div>
                    <div class="h-8 w-px bg-slate-200"></div>
                    <div
                        ...
                        <p
                            class="text-[11px] text-slate-400 font-semibold mb-0.5">
                            Duration</p>
                        <p class="font-bold text-blue-600 text-sm font-headline"
                            x-text="selectedProtocol?.duration_hours + 'h'"></p>
                    </div>
                    <span
                        class="material-symbols-outlined text-xs text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity ml-1">edit</span>
                </div>
                ...
                <div x-show="activeEditField === 'window'"
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label
                                class="text-[10px] font-bold text-slate-500 block mb-1">Start
                                Hour</label>
                            <input type="text" x-model="editForm.start_time"
                                @input="editForm.duration_hours = calcDuration(editForm.start_time, editForm.end_time)"
                                placeholder="18:00"
                                class="w-full px-3 py-2 text-xs border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        </div>
                        <div>
                            <label
                                class="text-[10px] font-bold text-slate-500 block mb-1">End
                                Hour</label>
                            <input type="text" x-model="editForm.end_time"
                                @input="editForm.duration_hours = calcDuration(editForm.start_time, editForm.end_time)"
                                placeholder="10:00"
                                class="w-full px-3 py-2 text-xs border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        </div>
                    ...
                    <div>
                        <label
                            class="text-[10px] font-bold text-slate-500 block mb-1">Duration
                            (Auto Calculated)</label>
                        <input type="number" min="1" max="168"
                            x-model.number="editForm.duration_hours" readonly
                            class="w-full px-3 py-2 text-xs border rounded-lg bg-slate-100 text-slate-700 font-bold outline-none cursor-not-allowed">
                    </div>
                    <div class="flex justify-end gap-2 pt-1">
                        <button type="button" @click="closeEdit()"
                            class="px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-200 rounded-lg">Batal</button>
            ...
            <section>
                <div class="flex justify-between items-center mb-2">
                    <label
                        class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active
                        Days</label>
                </div>

                <!-- Display Mode -->
                <div x-show="activeEditField !== 'days'"
                    @click="openEdit('days')"
                    ...
                    <div
                        class="flex items-center justify-between flex-wrap gap-1.5">
                        <div class="flex flex-wrap gap-1.5">
                            <template x-for="day in selectedProtocol?.days"
                                :key="'pday-' + day.day">
                                <span
                                    class="bg-blue-600 text-white text-[11px] px-3 py-1 rounded-lg font-bold shadow-sm"
                                    x-text="getDayLabel(day.day)"></span>
                            </template>
                        </div>
                        <span
                            class="material-symbols-outlined text-xs text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">edit</span>
                ...
                <div x-show="activeEditField === 'days'"
                    ...
                    <div class="grid grid-cols-7 gap-1">
                        <template x-for="d in [1,2,3,4,5,6,7]"
                            :key="'btn-d-' + d">
                            <button type="button" @click="toggleDay(d)"
                                :class="editForm.days.includes(d) ?
            ...
            <section>
                <div class="flex justify-between items-center mb-2">
                    <label
                        class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Clinical
                        Description</label>
                </div>

                <!-- Display Mode -->
                <div x-show="activeEditField !== 'description'"
                    ...
                    <div class="flex justify-between items-start">
                        <p class="text-xs text-slate-700 leading-relaxed font-body italic pr-4"
                            x-text="selectedProtocol?.description || 'Tidak ada deskripsi klinis'">
                        </p>
                        <span
                            class="material-symbols-outlined text-xs text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">edit</span>
                    </div>
                ...
                <div x-show="activeEditField === 'description'"
                    ...
                    <div>
                        <textarea rows="3" x-model="editForm.description"
                            class="w-full px-3 py-2 text-xs border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
    ...
    <div x-show="!selectedProtocol"
        ...
        <p class="text-xs">Silakan pilih protokol di sebelah kiri untuk melihat
            pratinjau.</p>
    </div>
</div>
